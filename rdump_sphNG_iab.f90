...
SUBROUTINE rdump_sphNG_iab(inputfile,rcheck, skip)
...
! Open file to read in:
      print*, '1) SPH file    : ', TRIM(inputfile)
      open(10,file=inputfile,status='old',iostat=check,&
...
! If open statement fails, return an error to main program:
      ...
      if(nblocks>1) npart = nparttot
      close(10)

      print*, 'NELEMENT ', npart

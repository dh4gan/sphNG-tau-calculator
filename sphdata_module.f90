module sphdata
  ...
  !-----------------------------------------------------------------------
  ! Data module for SPH simulations
  ! PJC 20/05/2008
  ! DHF 01/09/2009
...
! Integers
  ...
  integer(kind=1),allocatable,dimension(:) :: iphase,iorig
  
  integer(kind=8), allocatable,dimension(:) :: iunique
  ! Reals
  real(kind=8),allocatable,dimension(:) :: spinx,spiny,spinz
  ...
  integer(kind=2),allocatable :: radneigh(:)
  real, allocatable :: e(:), rkappa(:), cv(:), rlambda(:), edd(:)
  real, allocatable :: force(:,:)
  real(kind=4), allocatable :: dlnTdlnP(:), adiabaticgradient(:)
  real(kind=4), allocatable :: pressure(:,:), viscosity(:,:), gravity(:,:), radpres(:,:)
